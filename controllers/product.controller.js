...
exports.searchProducts = (req, res)=>{
    ...
    **********************************************************************************************/
    ...
    var direction = req.query.direction != undefined ? req.query.direction === "DESC" ? -1 : 1 : -1;

    //the properties to be sort if undefined or "" then sortby productId
    var sortName = req.query.sortBy != undefined? req.query.sortBy === "" ? 'productId': req.query.sortBy :'productId';
    ...
    Products.find(queryData).sort([[sortName, direction]])
    .then(response=>{
...
exports.deleteProduct = (req, res)=>{
    ...
    Products.findOne({_id: req.params.id}).then(product=>{
        if(product == null){
            res.setHeader('Content-Type', 'application/json');
            res.status(httpStatus.StatusCodes.BAD_REQUEST)
            .json({message: `No Product found for ID -${req.params.id}!`}).end();
            return;
        }
