...
exports.signUp = (req, res)=>{
    ...
    if(!isEmailValid){
        ...
        res.status(httpStatus.StatusCodes.BAD_REQUEST).json({message: "Invalid email-id format!"}).end();
        return;
    }
    ...
    if(!validateContact(req.body.contactNumber)){
        ...
        res.setHeader('Content-Type','application/json')
        res.status(httpStatus.StatusCodes.BAD_REQUEST).json({message: "Invalid contact number!"}).end();
        return;     
    }
        ...
        .then(hashPassword=>{
            data.password = hashPassword;
                ...
                .catch(err=>{
                    ...
                    console.log(err);
                    res.setHeader('Content-Type', 'application/json');
                    res.status(httpStatus.StatusCodes.BAD_REQUEST).json({message: "error while inserting"}).end();
                });  
        })
        .catch(err=>{
            ...
            console.log(err);
             ...
             res.status(httpStatus.StatusCodes.BAD_REQUEST).json({"message": "error encryptin password"}).end();
        })
    ...
    .catch(err=>{
        ...
        console.log(err);
        res.setHeader('Content-Type', 'applicatin/json');
        res.status(httpStatus.StatusCodes.BAD_REQUEST).json({"message": "This is the error"}).end();
    })
}
