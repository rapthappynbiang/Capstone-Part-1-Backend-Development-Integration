...
exports.saveOrder = (req,res)=>{
    ...
    //find the products
    Products.find({productId:  req.body.productId}).then(product=>{
        ...
        if(product.availableItems === 0){
            res.setHeader('Content-Type','application/json')
            res.status(httpStatus.StatusCodes.NOT_FOUND)
            .json({message: `Product with ID - <${req.body.productId}> is currently out of stock!`})
            .end();
            return;  
        }
        //else store the product and calculate price
        order.product = product;
        order.amount = product.price*req.body.quantity==undefined?1:req.body.quantity;

        //find the user Shipping address
        Addresses.findOne({_id: req.body.addressId}).then(address=>{
